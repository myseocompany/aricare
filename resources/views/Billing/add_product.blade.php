	...
	<tbody>
		...
		<form action="/order/products/store"  method="POST">
			...
			<tr>
				...
				<td class="text-center "><input type="text" id="product" name="product" value="" size="40" required="required">
		...
		@if(isset($model->productList)) 
			...
			<tr>
				<td>@if(isset($item->product)){{$item->product->name}}@endif
		...
		<tr>
			...
			<td colspan="3" class=" display-4 text-right ">$ {{number_format($model->getTotal(), 0)}}</td>
			<td></td>
		</tr>
		<tr>
			<td colspan="6">
